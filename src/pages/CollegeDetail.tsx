...
export default function CollegeDetail() {
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(saved ? `${API}/api/profile/unsave` : `${API}/api/profile/save`, {
      method:"POST",
  ...
  if (!college) {
    return (
      <div className="min-h-screen flex flex-col" style={{ fontFamily:BODY }}>
        <Header />
        <div className="flex-1 flex items-center justify-center flex-col gap-4 text-slate-500">
          <p className="text-xl font-bold" style={{ fontFamily:DISPLAY }}>College not found</p>
  ...
  return (
    <div className="min-h-screen bg-[#f6f7f8] text-slate-900 antialiased" style={{ fontFamily:BODY }}>
      <style>{`
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-2">
        <nav className="flex items-center gap-2 text-xs text-slate-400 font-medium">
          <Link to="/" className="hover:text-[#0b2647]">Home</Link><span>›</span>
          <Link to="/colleges" className="hover:text-[#0b2647]">Colleges</Link><span>›</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
        ...
        <div className="relative overflow-hidden rounded-2xl mb-6 shadow-2xl min-h-[200px]">
          ...
          <div className="absolute inset-0" style={{ background:"linear-gradient(135deg,rgba(11,38,71,0.93) 0%,rgba(26,74,138,0.86) 100%)" }} />
          <div className="relative z-10 p-6 md:p-10">
            <div className="flex flex-wrap gap-2 mb-4">
              {college.approvedBy.map(b => (
                <span key={b} className="px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-[10px] font-bold tracking-wider uppercase border border-white/20 text-white" style={{ fontFamily:DISPLAY }}>
                  {b}
                </span>
              ))}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-blue-200 mb-6 text-sm">
              <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">location_on</span>{college.city}</span>
              <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">calendar_today</span>Est. {college.established}</span>
              <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm text-yellow-400">star</span>
                <span className="font-bold text-white mono">{college.rating}</span><span className="text-blue-200"> Rating</span>
              </span>
              <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">workspace_premium</span>
                <span className="mono">Rank #{college.ranking}</span>
              </span>
              <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${typeColor[college.type]}`} style={{ fontFamily:DISPLAY }}>{college.type}</span>
            </div>
            <div className="flex flex-wrap gap-3">
              ...
              <button onClick={() => { navigator.clipboard.writeText(window.location.href); setShowCopied(true); setTimeout(()=>setShowCopied(false),2000); }}
                className="flex items-center gap-2 border-2 border-white/30 hover:bg-white/10 text-white px-5 py-3 rounded-xl font-bold transition-all text-sm"
                style={{ fontFamily:DISPLAY }}>
                <span className="material-symbols-outlined text-sm">link</span>{showCopied?"Copied!":"Copy Link"}
              </button>
              <a href={`https://wa.me/?text=Check out ${encodeURIComponent(college.name)} on CollegeMatch: ${encodeURIComponent(window.location.href)}`}
            ...
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => scrollTo(tab.id)}
                className={`px-6 py-4 text-sm font-semibold whitespace-nowrap transition-colors border-b-2 ${
                  activeTab===tab.id ? "border-[#0b2647] text-[#0b2647] bg-[#0b2647]/5" : "border-transparent text-slate-500 hover:text-[#0b2647] hover:bg-slate-50"
                }`}
                style={{ fontFamily:DISPLAY }}>
                {tab.label}
              </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-12">
            ...
            <section id="section-about">
              <h2 className="text-2xl font-bold mb-4 flex items-center gap-2" style={{ fontFamily:DISPLAY }}>
                <span className="material-symbols-outlined text-[#0b2647]">info</span>About
              </h2>
              <p className="text-slate-600 leading-relaxed mb-4">{college.description}</p>
              <p className="text-slate-600 leading-relaxed">
                Affiliated with <strong>{college.affiliation}</strong>, shaping careers since <strong>{college.established}</strong>. {college.courses.length} programs available.
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
                {[
                  ...
                  <div key={label} className="stat-card rounded-xl p-4 text-center">
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1" style={{ fontFamily:DISPLAY }}>{label}</p>
                    <p className="text-xl font-extrabold text-[#0b2647] mt-0.5 mono">{value}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section id="section-courses">
              <h2 className="text-2xl font-bold mb-6 flex items-center gap-2" style={{ fontFamily:DISPLAY }}>
                <span className="material-symbols-outlined text-[#0b2647]">menu_book</span>Courses Offered
              </h2>
              <div className="space-y-4">
                {college.courses.map(course => (
                  <div key={course.id} className="course-card p-5 md:p-6 bg-white rounded-xl border border-slate-200 transition-shadow">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div>
                        <h3 className="text-base font-bold text-slate-900" style={{ fontFamily:DISPLAY }}>{course.name}</h3>
                        <div className="flex flex-wrap gap-3 mt-1.5 text-sm text-slate-500">
                          ...
                          <span>·</span>
                          <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">groups</span>{course.seats} seats</span>
                          <span>·</span>
                          <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">grade</span>Cutoff: <span className="mono">{course.cutoffMarks}%</span></span>
                        </div>
                        {course.specializations && course.specializations.length>0 && (
                          <div className="flex flex-wrap gap-1.5 mt-2">
                            {course.specializations.map(s => (
                              <span key={s} className="text-xs bg-[#0b2647]/5 text-[#0b2647] px-2 py-0.5 rounded-md font-semibold" style={{ fontFamily:DISPLAY }}>{s}</span>
                            ))}
                          </div>
                        )}
                      </div>
                      <div className="md:text-right shrink-0">
                        <p className="text-xl font-extrabold text-blue-600 mono">{fmtFee(course.fees)}/yr</p>
                        {college.applicationLink && (
                          <a href={college.applicationLink} target="_blank" rel="noopener noreferrer"
            ...
            <section id="section-placements">
              <h2 className="text-2xl font-bold mb-6 flex items-center gap-2" style={{ fontFamily:DISPLAY }}>
                <span className="material-symbols-outlined text-[#0b2647]">trending_up</span>Placement Highlights
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                {[
                  ...
                  <div key={label} className="stat-card rounded-xl p-6">
                    <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1" style={{ fontFamily:DISPLAY }}>{label}</p>
                    <p className="text-3xl font-extrabold text-[#0b2647] mono">{value}</p>
                  </div>
                ))}
              </div>
              <div className="bg-slate-50 rounded-xl p-6 border border-slate-100 mb-6">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-5" style={{ fontFamily:DISPLAY }}>Top Recruiters</p>
                <div className="flex flex-wrap gap-3">
                  {college.placement.topRecruiters.map(r => (
                    <span key={r} className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 shadow-sm" style={{ fontFamily:DISPLAY }}>{r}</span>
                  ))}
                </div>
              </div>
              <div className="bg-white rounded-xl p-6 border border-slate-100 shadow-sm">
                <h3 className="text-base font-bold text-slate-800 mb-4 flex items-center gap-2" style={{ fontFamily:DISPLAY }}>
                  <span className="material-symbols-outlined text-[#0b2647]">show_chart</span>5-Year Placement Trend
                </h3>
            ...
            <section id="section-facilities">
              <h2 className="text-2xl font-bold mb-6 flex items-center gap-2" style={{ fontFamily:DISPLAY }}>
                <span className="material-symbols-outlined text-[#0b2647]">apartment</span>Campus Facilities
              </h2>
          ...
          <aside className="space-y-6">
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
              <h3 className="text-base font-bold mb-4" style={{ fontFamily:DISPLAY }}>Quick Facts</h3>
              <ul className="space-y-4">
                {[
                  ...
                  <li key={label} className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider" style={{ fontFamily:DISPLAY }}>{label}</p>
                      <p className="text-sm font-semibold text-slate-800 mono">{value}</p>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
              <h3 className="text-base font-bold mb-4" style={{ fontFamily:DISPLAY }}>Accreditations</h3>
              <div className="flex flex-wrap gap-2">
                {college.approvedBy.map(a => (
                  <span key={a} className="px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full text-xs font-bold" style={{ fontFamily:DISPLAY }}>✓ {a}</span>
                ))}
              </div>
            </div>

            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
              <h3 className="text-base font-bold mb-4" style={{ fontFamily:DISPLAY }}>Fee Range</h3>
              <div className="space-y-2.5">
                {college.courses.slice(0,5).map(c => (
                  <div key={c.id} className="flex justify-between items-center">
                    <span className="text-xs text-slate-600 truncate mr-2">{c.name.replace("B.Tech ","").replace("B.E. ","").replace("M.Tech ","M.")}</span>
                    <span className="text-xs font-bold text-[#0b2647] shrink-0 mono">{fmtFee(c.fees)}/yr</span>
                  </div>
                ))}
                {college.courses.length>5 && <p className="text-xs text-slate-400">+{college.courses.length-5} more</p>}
              </div>
            </div>

            <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
              <h3 className="text-base font-bold mb-4" style={{ fontFamily:DISPLAY }}>Planning Tools</h3>
              <div className="space-y-1">
                {[
                  ...
                  <Link key={tool.to} to={tool.to}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-indigo-50 transition text-sm font-medium text-slate-700 hover:text-[#0b2647]"
                    style={{ textDecoration:"none", fontFamily:DISPLAY }}>
                    <span className="material-symbols-outlined text-[#0b2647] text-lg">{tool.icon}</span>{tool.label}
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#0b2647] text-white p-6 rounded-xl shadow-lg">
              <h3 className="text-base font-bold mb-2" style={{ fontFamily:DISPLAY }}>Need help deciding?</h3>
              <p className="text-blue-200 text-sm mb-5 leading-relaxed">Let our AI match you with the best programs based on your profile.</p>
              <Link to="/recommend"
                ...
                style={{ textDecoration:"none", fontFamily:DISPLAY }}>
                <span className="material-symbols-outlined text-sm">auto_awesome</span>Get AI Recommendation
              </Link>
            </div>
          </aside>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-slate-200 py-3 px-4 shadow-2xl">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-sm" style={{ fontFamily:DISPLAY }}>Not sure if this is the right fit?</p>
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            <Link to="/recommend"
              ...
              style={{ textDecoration:"none", fontFamily:DISPLAY }}>
              Get Recommendation <span className="material-symbols-outlined text-sm">auto_awesome</span>
            </Link>
          </div>
        </div>
      </div>
